...
int quickSort(int *a, int n) //퀵정렬 함수
{
	...
	if (n > 1) //배열의 크기가 1보다 큰 경우 
	{
		v = a[n-1]; //v에 a[n-1]값 복사
		i = -1;
		j = n - 1; //j에 인덱스 n-1 값 복사

		while(1)
		{
			while(a[++i] < v);//a[i]가 v보다 커질 때까지 반복
			while(a[--j] > v);//a[j]가 v보다 작아질 때까지 반복

			if (i >= j) break;
			t = a[i];//t에 a[i]복사
			a[i] = a[j];//a[i]에 a[j]복사
			a[j] = t;//a[j]에 t 복사
		}
		t = a[i];//t에 a[i] 복사
		a[i] = a[n-1]; //a[i]에 a[n-1]복사
		a[n-1] = t; // a[n-1]에 t 복사
        //두개의 리스트를 각각 퀵정렬
		quickSort(a, i); 
...
int hashCode(int key) { //hashcode함수 실행
   return key % MAX_HASH_TABLE_SIZE; //key값을 해시테이블의 크기의 나머지값 리턴
}

int hashing(int *a, int **ht)
{
	int *hashtable = NULL; //NULL로 초기화
	...
	for(int i = 0; i < MAX_HASH_TABLE_SIZE; i++) //해시테이블 값 -1로 초기화
		hashtable[i] = -1;
	...
	*/
	...
	for (int i = 0; i < MAX_ARRAY_SIZE; i++)
	{
		key = a[i]; //key값에 a[i] 대입
		hashcode = hashCode(key);//해쉬코드에 해쉬코드 함수 실행 후 값 복사
		/*
		printf("key = %d, hashcode = %d, hashtable[%d]=%d\n", key, hashcode, hashcode, hashtable[hashcode]);
		*/
		if (hashtable[hashcode] == -1) //해쉬테이블이 빈 경우
		{
			hashtable[hashcode] = key; //key값 대입
		} else 	{ //해쉬테이블에 값이 존재하는 경우 

			index = hashcode; //인덱스에 해시코드 대입

			while(hashtable[index] != -1)//해시테이블이 비어있는 경우를 찾을 때까지 반복
			{
				index = (++index) % MAX_HASH_TABLE_SIZE; //인덱스 값을 변경하면서 비어있는 테이블 찾기
				/*
				printf("index = %d\n", index);
				*/
			}
			hashtable[index] = key;//빈 해시 테이블을 찾은 경우 key값 대입
		}
	}
...
int search(int *ht, int key) //search 함수
{
	int index = hashCode(key); // index에 해시코드 key값 대입

	if(ht[index] == key) //인덱스에 존재하는 값이 key값과 일치하면 index 리턴
		return index;

	while(ht[++index] != key)//index값을 변경하면서 key 값과 같을 때까지 반복 
	{
		index = index % MAX_HASH_TABLE_SIZE; //인덱스 값을 변경하면서 key값과 같아진 경우
	}
	return index; //인덱스 리턴
}
